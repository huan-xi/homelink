...
use dashmap::DashMap;
use log::error;
use sea_orm::{ActiveModelTrait, ColumnTrait, DatabaseConnection, DatabaseTransaction, EntityTrait, NotSet, QueryFilter, TransactionTrait, TryIntoModel};
...
pub struct TemplateManagerInner {
    ...
    ///model 和模板的映射
    pub mihome_templates: DashMap<String, HlDeviceTemplate>,
}
...
impl TemplateManagerInner {
    ...
    pub async fn apply_mihome_template(&self, conn: &DatabaseConnection, model: &MiotDeviceModel, option: &ApplyTemplateOptions) -> anyhow::Result<()> {
        let temp = self.mihome_templates
